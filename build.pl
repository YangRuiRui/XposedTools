...
sub main() {
    ...
    if ($action eq 'build') {
        my $jar = $Xposed::cfg->val('General', 'outdir') . '/java/PmxpedBridge.jar';
        if (!-r $jar) {
...
Values for <steps> are provided as a comma-separated list of:
  ...
  prop      Create the pmxped.prop file.
...
sub create_xposed.prop($$;$) {
    ...
    # Open the file
    my $coldir = Xposed::get_collection_dir($platform, $sdk);
    my $propfile = $coldir . '/files/system/pmxped.prop';
    print "$propfile\n";
...
sub create_zip($$) {
    ...
    $zip->addFile("$outdir/java/PmxpedBridge.jar", 'system/framework/PmxpedBridge.jar') || return 0;
...
sub build_java() {
    ...
    print_status('Copying APK to PmxpedBridge.jar...', 1);
    my $base = $javadir . '/app/build/outputs/apk/app-release';
    foreach my $suffix ('.apk', '-unaligned.apk', '-unsigned.apk') {
        my $file = $base . $suffix;
        if (-f $file) {
            my $target = $Xposed::cfg->val('General', 'outdir') . '/java/PmxpedBridge.jar';
            print "$file => $target\n";
            make_path(dirname($target));
            if (!copy($file, $target)) {
                print_error("Copy failed: $!");
